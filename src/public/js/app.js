...
function renderPathFilter() {
  ...
  select.innerHTML = '<option value="">所有路径</option>' + 
...
function initCharts() {
  ...
  if (taskCanvas) {
    ...
    // 统计任务状态
    const statusCount = { done: 0, in_progress: 0, waiting: 0, cancelled: 0, paused: 0 };
...
function filterProjects(projects) {
  return projects.filter(project => {
    ...
    // 状态过滤 - 筛选要显示的项目
    if (AppState.currentFilter === 'done') {
